        ...
        __global uint matrix[$n_labels][$n_labels])
{
    ...
    for (size_t j = 0; j < seeds_per_thread; j++) {
        ...
        int start_label = nearest_vertex_label(vertices, vertex_labels, start_location, distance_upper_bound);
        int end_label = nearest_vertex_label(vertices, vertex_labels, location, distance_upper_bound);
