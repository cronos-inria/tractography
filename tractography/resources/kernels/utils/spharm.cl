...
/**
 * EVALUATE_SH
 * Evaluate the spherical harmonics Ylm(phi, theta) and their derivatives at specific angles.
**/
void evaluate_sh(float phi, float theta, float ylm[45], float ylm_dp[45], float ylm_dt[45]) {
...
/**
 * EVALUATE_SH_DPHI_OVER_SIN_THETA
 * Evaluate the spherical harmonics Ylm(phi, theta) and their derivatives at specific angles.
 *
 * IMPORTANT: The derivative with respect to phi is pre-divided by sin(theta) to avoid
 * singularities at the poles (theta = 0 or pi) during integration.
**/
void evaluate_sh_dphi_over_sin_theta(float phi, float theta, float ylm[45], float ylm_dp[45], float ylm_dt[45]) {
    ...
    ylm_dp[1] = 0.54627421529f * st * 2.0f * c2p;
    ylm_dp[2] = -1.0925484305920792f * ct * cp;
    ylm_dp[3] = 0.0f; 
    ylm_dp[4] = 1.0925484305920792f * ct * sp;
    ylm_dp[5] =  -0.54627421529f * st * 2.0f * s2p;

    ylm_dp[6] = 0.62583573544f * st3 * 4.0f * c4p;
    ylm_dp[7] = -1.77013076978f * st2 * ct * 3.0f * c3p;
    ylm_dp[8] = 0.47308734787f * st * (7.0f * ct2 - 1.0f) * 2.0f * c2p;
    ylm_dp[9] = -0.66904654355f * ct * (7.0f * ct2 - 3.0f) * cp;
    ylm_dp[10] = 0.0f;
    ylm_dp[11] = 0.66904654355f * ct * (7.0f * ct2 - 3.0f) * sp;
    ylm_dp[12] = -0.47308734787f * st * (7.0f * ct2 - 1.0f) * 2.0f * s2p;
    ylm_dp[13] = 1.77013076978f * st2 * ct * 3.0f * s3p;
    ylm_dp[14] = -0.62583573544f * st3 * 4.0f * s4p;

	ylm_dp[15] = 0.68318410519f * st5 * 6.0f * c6p;
	ylm_dp[16] = -2.36661916223f * st4 * ct * 5.0f * c5p;
	ylm_dp[17] = 0.50456490072f * st3 * (11.0f * ct2 - 1.0f) * 4.0f * c4p;
	ylm_dp[18] = -0.92120525951f * st2 * (11.0f * ct3 - 3.0f * ct) * 3.0f * c3p;
	ylm_dp[19] = 0.46060262975f * st * (33.0f * ct4 - 18.0f * ct2 + 1.0f) * 2.0f * c2p;
	ylm_dp[20] = -0.58262136251f * (33.0f * ct5 - 30.0f * ct3 + 5.0f * ct) * cp;
	ylm_dp[21] = 0.0f; 
	ylm_dp[22] = 0.58262136251f * (33.0f * ct5 - 30.0f * ct3 + 5.0f * ct) * sp;
	ylm_dp[23] = -0.46060262975f * st * (33.0f * ct4 - 18.0f * ct2 + 1.0f) * 2.0f * s2p;
	ylm_dp[24] = 0.92120525951f * st2 * (11.0f * ct3 - 3.0f * ct) * 3.0f * s3p;
	ylm_dp[25] = -0.50456490072f * st3 * (11.0f * ct2 - 1.0f) * 4.0f * s4p;
	ylm_dp[26] = 2.36661916223f * st4 * ct * 5.0f * s5p;
	ylm_dp[27] = -0.68318410519f * st5 * 6.0f * s6p;

	ylm_dp[28] = 0.72892666017f * st7 * 8.0f * c8p;
	ylm_dp[29] = -2.9157066407f * st6 * ct * 7.0f * c7p;
	ylm_dp[30] = 0.53233276606f * st5 * (15.0f * ct2 - 1.0f) * 6.0f * c6p;
	ylm_dp[31] = -3.4499106221f * st4 * (5.0f * ct3 - ct) * 5.0f * c5p;
	ylm_dp[32] = 0.47841652475f * st3 * (65.0f * ct4 - 26.0f * ct2 + 1.0f) * 4.0f * c4p;
	ylm_dp[33] = -1.2352661553f * st2 * (39.0f * ct5 - 26.0f * ct3 + 3 * ct) * 3.0f * c3p;
	ylm_dp[34] = 0.45615225843f * st * (143.0f * ct6 - 143.0f * ct4 + 33.0f * ct2 - 1.0f) * 2.0f * c2p;
	ylm_dp[35] = -0.10904124589f * (715.0f * ct7 - 1001.0f * ct5 + 385.0f * ct3 - 35.0f * ct) * cp;
	ylm_dp[36] = 0.0f; 
	ylm_dp[37] = 0.10904124589f * (715.0f * ct7 - 1001.0f * ct5 + 385.0f * ct3 - 35.0f * ct) * sp;
	ylm_dp[38] = -0.45615225843f * st * (143.0f * ct6 - 143.0f * ct4 + 33.0f * ct2 - 1.0f) * 2.0f * s2p;
	ylm_dp[39] = 1.2352661553f * st2 * (39.0f * ct5 - 26.0f * ct3 + 3 * ct) * 3.0f * s3p;
	ylm_dp[40] = -0.47841652475f * st3 * (65.0f * ct4 - 26.0f * ct2 + 1.0f) * 4.0f * s4p;
	ylm_dp[41] = 3.4499106221f * st4 * (5.0f * ct3 - ct) * 5.0f * s5p;
	ylm_dp[42] = -0.53233276606f * st5 * (15.0f * ct2 - 1.0f) * 6.0f * s6p;
	ylm_dp[43] = 2.9157066407f * st6 * ct * 7.0f * s7p; 
	ylm_dp[44] = -0.72892666017f * st7 * 8.0f * s8p; 
	...
	global float values[100][45])
{
	...
	for (size_t i = 0; i < n_values; i++) {
		evaluate_sh(azimuth[i], colatitude[i], ylm, ylm_dp, ylm_dt);
